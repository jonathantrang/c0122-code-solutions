...
2. Figure out how to reach output seperate from code
  How to get get from LearningFuze -> False or burgers -> True
...
3. Write down the list of steps necessary to replicate the process and test
  ...
  e. What happens when the system repeats?
    ...
    - If the letter is uppercase it will immediately go to false
  f. How to exit?
...
function isLowerCased(word) {
  ...
  for (var i = 0; i < word.length; i++) {
    if (word.charAt(i) === '-') {
      continue;
    } else if (word.charAt(i) === word[i].toUpperCase()) {
      return false;
    }
  }
  return result;
